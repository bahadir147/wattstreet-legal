...
<h3>g) Compras dentro de la app</h3>
<p>El juego ofrece compras opcionales dentro de la app (por ejemplo Sin anuncios, un Pack de inicio, ingresos ×2 permanentes y packs de patentes). Los pagos los procesan íntegramente Google Play o el App Store de Apple; nunca recibimos los datos de tu tarjeta o cuenta bancaria. El juego solo recibe de la tienda el token o recibo de compra (ID del producto, ID del pedido o transacción y fecha de compra) para verificar la compra en tu dispositivo, entregarte lo que compraste y restaurarlo más adelante. Un registro de tus compras (IDs de producto y transacción) se guarda en tu partida y en su copia en la nube para que sobreviva a una reinstalación. Es historial de compras usado solo para el funcionamiento de la app.</p>
...
<p>No recopilamos tu nombre, correo (salvo que nos escribas), teléfono, contactos, fotos o archivos, ubicación precisa o aproximada mediante los servicios de ubicación del dispositivo, ni datos de cámara o micrófono. El juego no tiene chat ni funciones sociales. No usamos el servicio Unity Analytics. Si el juego se cierra inesperadamente o sufre un error, se envía a Unity (Cloud Diagnostics) un informe de fallo (modelo del dispositivo, sistema operativo, versión de la app y el registro técnico del error, sin datos personales) para que podamos corregir errores. El propio motor Unity puede enviar a Unity información técnica limitada (por ejemplo, modelo del dispositivo, sistema operativo y versión del motor), como se describe en la <a href="%U_PRIV%">política de privacidad de Unity</a>.</p>"""),
...
("Compras", """<p>El juego ofrece compras opcionales dentro de la app. Se realizan a través de Google Play o el App Store de Apple y se rigen por sus condiciones y políticas de reembolso. Los artículos únicos (Sin anuncios, Pack de inicio, ingresos ×2) se pueden restaurar con “Restaurar compras” en la Tienda del juego; los packs de patentes se consumen al entregarse. Tus derechos legales como consumidor no se ven afectados.</p>"""),
